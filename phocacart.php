...
class plgPhocaPDFPhocaCart extends CMSPlugin
{
	...
	function onBeforeDisplayPDFPhocaCart(&$pdf, &$content, &$document, $staticData = '') {
		...
		// EAN Code
		$regex_one		= '/({phocapdfeancode\s*)(.*?)(})/si';
		$regex_all		= '/{phocapdfeancode\s*.*?}/si';
		$matches 		= array();
		$count_matches	= preg_match_all($regex_all,$documentOutput,$matches,PREG_OFFSET_CAPTURE | PREG_PATTERN_ORDER);
		...
		if ($count_matches != 0) {
			for($i = 0; $i < $count_matches; $i++) {
				...
				if (isset($parts[1]) && $parts[1] != '') {
					...
					$params = $pdf->serializeTCPDFtagParameters(array($text, 'EAN13', '', '', 80, 16, '', $style, 'N'));

					$output = '<tcpdf method="write1DBarcode" params="'.$params.'" />';
...
class PhocaPDFPhocaCartTCPDF extends TCPDF
{
	...
	public function Footer() {
		...
		if ($params['footer_display'] == 1) {
			...
			if (!isset($this->l['w_page'])) { $this->l['w_page'] = '';}
